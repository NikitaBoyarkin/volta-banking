...
PYTHONPATH=.:scripts uv run python scripts/generate_ab_data.py
...
# Run analyses (from repo root; scripts/ + root on PYTHONPATH for utils/)
...
### Seventeen-Project Portfolio

All analysis scripts, generators, and `build_notebooks.py` live in `scripts/`;
`utils/` stays at repo root. Run them with `PYTHONPATH=.:scripts` (the Makefile
`PY` variable does this) so both `scripts/` modules and `utils/` resolve.
...
17. **`volta_referral_segments.py`** — referral conversion by JTBD segment
   (Market & Jobs layer, Sprint 6 T6). Loads `volta_referral_segments.csv`
   (generated by `generate_referral_segments_data.py` — segment-dependent
   referral funnel). Referral funnel (sent→accepted→KYC→first-tx) by segment,
   chi-square segment × status, two-proportion z-test anchor vs gap, channel
   effect within gap segments, bar chart PNG
   (`referral_conversion_by_segment.png`). Validates audit risk #5: referral
   doesn't scale to new segments — anchor 29.6% vs Digital Newcomers 4.8%.
...
- **Test suite + coverage.** `uv run pytest` runs 58+ tests including end-to-end
  ...
  default via `addopts`. `PYTHONPATH=.:scripts uv run python -c "import volta_X"`
