...
> **22 projects** | Funnel → A/B → Retention → Segmentation → Churn → RFM → CLV → Attribution → Anomaly → Spend → Support & Churn → NPS → JTBD×Cohorts → Unit Economics → Premium Upsell → 45+ KYC Deep-Dive → Referral Segments → Assisted CAC vs LTV → FX Sourcing → Segment Premium Offers → Anchor Launch CAC → Dormant Win-Back | Fintech / Digital Banking

Twenty-two projects follow one product from first install to retention: an
...
| 22 | **Dormant Win-Back** | Can assisted win-back recover dormant 45+? | Yes (UX barrier) — but only light-touch on 30–90d pays; human never does |
...
### Project 22 — Dormant 45+ Win-Back (RAT v2 layer)

- Three-arm randomized win-back for dormant Digital Newcomers 45+ — automated (control), **light-touch** (SMS + guided in-app + optional callback), and **human-assisted** (agent call) — across four dormancy buckets
- **Mechanism refuted on risk #5:** assisted reactivation DOES recover dormant 45+ — human **+5.34pp**, light-touch **+2.77pp** vs control overall (both significant), strongest on the recently dormant (30–60d human **+11.37pp**) — UX was the barrier, as Project 13 argued
- **Economics refine it:** full human calls lose money at every depth (overall ROI **0.40**); light-touch clears the gate overall (**1.08**) and strongly on 30–60d (**2.54**) and 60–90d (**1.26**)
- Break-even LTV per reactivated user: light-touch 30–60d €34 / 60–90d €59; human 30–60d €92 — above the segment's ~€85 LTV
- Implication: run a **light-touch assisted win-back targeted at the 30–90 day dormant**, reserve the human call for high-value/high-balance escalation. Refutes audit risk v2 #5 on mechanism, refines it on economics
...
**Dormant 45+ win-back** (Project 22)
![Reactivation by arm and dormancy](outputs/dormant_winback_reactivation.png)
![ROI by arm and dormancy](outputs/dormant_winback_roi.png)
...
│   ├── volta_dormant_winback.py      Project 22 — dormant 45+ win-back (RAT v2 #5)
...
- **Dormant Win-Back** — three-arm randomized win-back (automated / light-touch /
  human-assisted) across dormancy buckets, per-arm and per-bucket z-tests with
  Holm correction, ROI per 10K treated, targeting boundary, break-even LTV
