...
> **17 projects** | Funnel → A/B → Retention → Segmentation → Churn → RFM → CLV → Attribution → Anomaly → Spend → Support & Churn → NPS → JTBD×Cohorts → Unit Economics → Premium Upsell → 45+ KYC Deep-Dive → Referral Segments | Fintech / Digital Banking

Seventeen projects follow one product from first install to retention: an
...
actually caused the lift — then churn, segmentation, and a Market & Jobs (JTBD)
layer. Every headline number is reproducible from the seeded scripts in the repo.
...
| 16 | **45+ KYC Deep-Dive** | Does the KYC fix close 45+? | 45+ lift +0.6pp (ns) vs 35-44 +10pp — friction is trust, not UX |
...
- HTE cut of the KYC progress bar A/B by age: **35-44 +10pp** (p<0.001) and **18-24 +4.6pp** (p<0.05) lift significantly; **45+ +0.6pp** (p=0.82, ns) — the fix doesn't close 45+
- The 45+ gap persists in treatment: **45+ 52.4% vs 25-34 60.4%** (z-test significant) — even with the fix, 45+ converts worst
- Channel: **referral (trust) converts 45+ best** (63.3%) and has the smallest 45+ vs 25-34 gap (-2pp vs -10.7pp website) — friction is trust, not UX
